...
**Obiettivo (da PLAN.md):** frontend (HTML + canvas 2D + JS) servito dal backend Node. Il backend consuma `fleet_state` da Kafka e fa push in websocket; la pagina disegna la mappa a grafo e i robot live. Pannelli (incl. robot a rischio dalle previsioni) + casella di query NL verso l'endpoint TAG.
...
L'utente ha chiesto se dalla dashboard si potesse avviare una simulazione (scegliendo N robot e la topologia del magazzino), vedere tutti i robot in una tabella, e decidere quanti guasti iniettare e dove. Discusso lo scope con tre domande mirate prima di implementare (tre decisioni architetturali con costi molto diversi):
