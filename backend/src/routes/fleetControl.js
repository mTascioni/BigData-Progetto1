...
// repair_node/reserve_node: la dashboard li usa per etichettare lo stato di
// ciascun robot reale (in riparazione / di riserva / in servizio)
// confrontandoli col suo goal_node corrente. reserve_robot_ids (tutti i
// robot fleet senza una voce in tasks[], possono essere piu' di uno in
// scale=large) serve a riconoscere una riserva mai ancora dispacciata: non
// ha mai ricevuto un goal, quindi goal_node e' vuoto e non coincide ne' con
// repair_node ne' con reserve_node.
...
// Un robot reale "in avaria" (guasto persistente, gia' fermato da
// onPersistentFailure in fleetStateStore.js) viene tolto dalla flotta su
// azione esplicita dell'operatore -- sparisce da mappa/tabelle, non viene
// mai piu' scelto come riserva. Non ferma i processi ROS del suo namespace
// (v1 dichiarata, vedi fleetStateStore.js).
...
// La simulazione ROS/Gazebo non parte in automatico all'avvio dello stack --
// la dashboard la avvia/ferma da qui, scegliendo la scala (small/large).
// Stesso pattern start/stop/status del pannello del generatore sintetico
// (routes/generator.js).
...
router.post("/sim/start", async (req, res) => {
  try {
    ...
    // Un run reale nuovo riparte "pulito" anche lato backend, non solo lato
    // ROS/Gazebo -- altrimenti riserve/robot in riparazione di un run
    // precedente restano bloccati per sempre (vedi resetRealFleetState).
    resetRealFleetState();
    res.json(result);
  } catch (err) {
    res.status(err.status || 502).json({ error: err.message });
