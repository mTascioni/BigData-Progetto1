...
**Passo 15, estensione oltre il piano originale (2026-07-22):** cinque correzioni/estensioni nate dall'uso reale del sistema — (1) fix di un bug reale di falsi positivi nel livelock quando il `goal_node` cambia fra due campionamenti (soprattutto nel generatore sintetico, che ne assegna uno nuovo a caso molto più spesso di un robot ROS reale); (2) un quarto operatore Spark stateful (`type="previsione"`) che rileva in streaming un preavviso di guasto (nuovo tipo di guasto `preavviso_intermittente`, raffiche saltuarie) e attiva una riparazione preventiva, mentre un guasto persistente confermato ora ferma il robot dov'è invece di ripararlo in automatico, in attesa che l'operatore lo decommissioni dalla dashboard; (3) menu a tendina per scegliere il robot su cui iniettare un guasto nel generatore sintetico; (4) isolamento dei dati fra run diversi via `run_id` (non cancellazione fisica), propagato end-to-end fino a previsioni/query; (5) tre nodi di deposito dedicati (`DEPOSITO`/`RISERVA1`/`RISERVA2`) fuori dai corridoi principali, dopo aver trovato una collisione fisica reale fra `repair_node`/`reserve_node` e gli start_node di robot reali. Dettagli, verifica dal vivo e limiti in `docs/passi/15-previsione-live-deposito-run-id.md`.

**Passo 16, estensione oltre il piano originale (2026-07-22):** due richieste di interattività sulla GUI + tre bug corretti dopo altro uso reale del sistema — (1) sezione dedicata per iniettare "perturbazioni" (`rumore_sensore`, rumore extra su un canale, deliberatamente non un guasto: esclusa da `injected_faults` cosi' resta un falso positivo per `offline/adaptive_thresholds.py`), reattiva come i guasti già esistenti; (2) pannello "streaming live" in dashboard che mostra i messaggi grezzi campionati dei topic Kafka (`telemetry`/`anomalies`/`injected_faults`/`fleet_state`), nessuna persistenza; (3) fix di un bug reale per cui una riserva già usata (es. R4) non ricompariva più nei run successivi della flotta reale (stato in memoria del backend mai resettato fra un avvio e l'altro); (4) fix di un bug reale nel generatore sintetico che poteva far nascere/finire un robot-token sui nodi `DEPOSITO`/`RISERVA1`/`RISERVA2` per un normale random walk, senza nessun guasto; (5) fix del pannello "Robot a rischio", bloccato su un dato di test residuo e cieco alle previsioni già viste in tempo reale in "Eventi recenti". Dettagli, verifica dal vivo e limiti in `docs/passi/16-perturbazioni-streaming-live-fix-riserve.md`.
