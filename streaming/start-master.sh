...
echo "[start-master] master pronto, avvio query_service.py, detection_job.py e persistence_job.py"
...
(
  ...
  while true; do
    ...
    echo "[start-master] persistence_job.py terminato (exit $?), riavvio tra 5s..." >> /tmp/persistence_job.log
    sleep 5
  done
) &
