...
- ARIMA/Prophet o LSTM (Python) — previsione time-series dei guasti (offline)
- Parquet / Delta Lake — storico
- DuckDB — esecuzione dell'SQL del layer TAG sui Parquet
- Qwen-Coder via HuggingFace — text-to-SQL (no CrewAI)
- **Node.js (Express/Fastify) — backend + dashboard:** serve la pagina, consuma `fleet_state` via `kafkajs` e fa push in websocket, espone l'endpoint TAG (riusa lo script HF esistente), esegue l'SQL via `duckdb` (node)
...
Storage (directory Parquet): `telemetry/`, `anomalies/`, `injected_faults/`, `predictions/`.
...
self-healing-fleet/
  ...
  streaming/            # job PySpark di detection (real-time)
  ...
  eval/                 # script esperimenti + domande di riferimento
```
